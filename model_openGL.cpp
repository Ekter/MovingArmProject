...
void Cmodel_openGL::texture_load(const char * filename, int wrap, int texture_number, int texture_width, int texture_height)
{
	...
	// open texture data
	/* la commande suivante "file = fopen_s( filename, "rb" );" a été remplacé par la commande ci-dessous
	pour éviter l'avertissement C4996 concernant le risque de sécurité de la fonction "fopen" */
...
void Cmodel_openGL::draw_arm(void)
{
	...
	glPushMatrix();
		...
		glRectf(0.1f, 0.4f, 0.14f, -0.4f); // glRectf(coin supérieur gauche: x, y, coin inférieur droit: x, y)
